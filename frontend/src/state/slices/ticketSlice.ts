...
>(
    ...
    async (id, { rejectWithValue }) => {
        if (useMockTickets) {
            ...
            const found = mockTickets.find(t => t.requestId === id);
    ...
    async (body, { rejectWithValue }) => {
        if (useMockTickets) {
            ...
            const newMockTicket: Ticket = {
                ...
                requestNumber: `REQ-${Math.floor(Math.random() * 1000)}`,
                userId: "user_001",
...
>(
    ...
    async ({id, updates}, {rejectWithValue}) => {
        if (useMockTickets) {
            return { requestId: id, ...updates } as Ticket;
        }
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchTicketsThunk.rejected, (state, action) => {
                state.isLoadingList = false;
                const errMsg = mapTicketErrorCodeToMessage(action.payload ?? action.error.message);
                state.error = errMsg;
            ...
            .addCase(fetchTicketByIdThunk.rejected, (state, action) => {
                state.isLoadingCurrent = false;
                state.error = mapTicketErrorCodeToMessage(action.payload ?? action.error.message);
            })
            ...
            .addCase(updateTicketThunk.fulfilled, (state, action) => {
                ...
                const index = state.items.findIndex((t) => t.requestId === patch.requestId);
                if(index !== -1) {
